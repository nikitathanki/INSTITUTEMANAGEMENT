...
{
    public partial class FEEVIEW : Form
    {
        public FEEVIEW()
        {
            InitializeComponent();
        ...
        private void guna2Button8_Click(object sender, EventArgs e)
        {
                ...
                if (!string.IsNullOrWhiteSpace(txtstdid.Text))
                {
                    SqlConnection conn = new SqlConnection(cs);
                     ...
                     conn.Open();
                    SqlDataAdapter da = new SqlDataAdapter("SELECT stdid, course, sub, feecategory, amount FROM feemaster WHERE stdid = @stdid", conn);
                    da.SelectCommand.Parameters.AddWithValue("@stdid", txtstdid.Text);
                    ...
                    {
                    // Set column headers
                    guna2DataGridView1.Columns[0].HeaderText = "Student ID";
                    guna2DataGridView1.Columns[1].HeaderText = "Course";
                    guna2DataGridView1.Columns[2].HeaderText = "Subject";
                    guna2DataGridView1.Columns[3].HeaderText = "Fee Category";
                    guna2DataGridView1.Columns[4].HeaderText = "Amount";
                    }
                }
                else
                {
